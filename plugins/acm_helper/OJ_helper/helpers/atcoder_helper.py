...
import pytz
from requests import get as reqests_get


class AtCoderHelper(OJHelper):
    ...
    def get_data(self, username: str) -> str:
        ...
        url: str = 'https://atcoder.jp/users/{username}'.format(
            username=username)
        # 国外网站, 这里加上 proxy
        response = reqests_get(url, proxies=self.proxies)
    ...
    def gatgatInfo(self, username: str) -> list:
        ...
        return [[i.text()[5:-2] for i in doc('.dl-table.mt-2').children().siblings().items()][0]] + \
            [[i.text()[7:]
              for i in doc('.dl-table.mt-2').children().siblings().items()][1]]
    ...
    def gechapoints(self, days: int) -> list[ContestInfo]:
        ...
        contests = [j for i, j in enumerate([i.text() for i in doc(
            '#contest-table-upcoming').find('a').items()]) if (i+1) % 2 == 0]
        ...
        '''
        times = [j for i, j in enumerate([i.text() for i in doc(
            '#contest-table-upcoming .text-center').items()][4:]) if (i+1) % 3 != 0]
        data: list[ContestInfo] = []
        current_time = datetime.now(pytz.utc)
        for i in range(0, len(times), 2):
            start_time = datetime.strptime(
                times[i], '%Y-%m-%d %H:%M:%S%z')  # type: ignore
